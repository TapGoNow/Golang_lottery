...
func getGiftPrizeDataOneDay(num int) map[int][60]int {
	...
	// 奖品数量少的时候，或者剩下了一些没有分配，需要用到随机概率来计算
	for num > 0 {
		num--
		// 通过随机数确定奖品落在哪个小时
...
//清空发奖计划
func clearGiftPrizeData(giftInfo *models.LtGift, service services.GiftService) {
	info := &models.LtGift{
		Id:        giftInfo.Id,
		PrizeData: "",
	}
	err := service.Update(info, []string{"prize_data"})
	if err != nil {
...
// 重新整理优惠券的编码到缓存中
func RecacheCodes(id int, service services.CodeService) (sucNum, errNum int) {
	// 集群版本需要放入到redis中
	// [暂时]本机版本的就直接从数据库中处理吧
