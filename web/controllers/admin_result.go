...
func (c *AdminResultController) Get() mvc.Result {
	...
	if giftId > 0 {
		datalist = c.ResultService.SearchByGift(giftId, page, size)
	} else if uid > 0 {
		datalist = c.ResultService.SearchByUser(uid, page, size)
	} else {
		datalist = c.ResultService.GetAll(page, size)
	}
	...
	if len(datalist) >= size {
		if giftId > 0 {
			total = int(c.ResultService.CountByGift(giftId))
		} else if uid > 0 {
			total = int(c.ResultService.CountByUser(uid))
		} else {
			total = int(c.ResultService.CountAll())
		}
...
func (c *AdminResultController) GetCheat() mvc.Result {
	id, err := c.Ctx.URLParamInt("id")
	if err == nil {
		c.ResultService.Update(&models.LtResult{Id: id, SysStatus: 2}, []string{"sys_status"})
	}
	refer := c.Ctx.GetHeader("Referer")
	if refer == "" {
		refer = "/admin/result"
...
func (c *AdminResultController) GetReset() mvc.Result {
	id, err := c.Ctx.URLParamInt("id")
	if err == nil {
		c.ResultService.Update(&models.LtResult{Id: id, SysStatus: 0}, []string{"sys_status"})
	}
	refer := c.Ctx.GetHeader("Referer")
	if refer == "" {
		refer = "/admin/result"
