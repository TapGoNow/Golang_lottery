...
func (c *AdminGiftController) Get() mvc.Result {
	//数据列表
	datalist := c.GiftService.GetAll(false)
...
func (c *AdminGiftController) GetEdit() mvc.Result {
	...
	if id > 0 {
		data := c.GiftService.Get(id, false)
		if data != nil {
			giftInfo.Id = data.Id
...
func (c *AdminGiftController) PostSave() mvc.Result {
	...
	if giftInfo.Id > 0 {
		datainfo := c.GiftService.Get(giftInfo.Id, false)
		if datainfo != nil {
			...
			if datainfo.PrizeNum != giftInfo.PrizeNum {
				...
				utils.ResetGiftPrizeData(&giftInfo, c.GiftService)
			}
			if datainfo.PrizeTime != giftInfo.PrizeTime {
				// 发奖周期发生了变化
				utils.ResetGiftPrizeData(&giftInfo, c.GiftService)
			}
			c.GiftService.Update(&giftInfo, []string{"title", "prize_num", "left_num", "prize_code", "prize_time",
...
func (c *AdminGiftController) GetReset() mvc.Result {
	id, err := c.Ctx.URLParamInt("id")
	if err == nil {
		c.GiftService.Update(&models.LtGift{Id: id, SysStatus: 0}, []string{"sys_status"})
	}
	return mvc.Response{
		Path: "/admin/gift",
	}
