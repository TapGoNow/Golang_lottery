...
func (c *AdminCodeController) Get() mvc.Result {
	...
	if giftId > 0 {
		...
		datalist = c.CodeService.GetAll(page, size)
	}
...
func (c *AdminCodeController) PostImport() {
	...
	gift := c.GiftService.Get(giftId, true)
...
func (c *AdminCodeController) GetReset() mvc.Result {
	id, err := c.Ctx.URLParamInt("id")
	if err == nil {
		c.CodeService.Update(&models.LtCode{Id: id, SysStatus: 0}, []string{"sys_status"})
	}
	return mvc.Response{
		Path: "/admin/code",
	}
...
func (c *AdminCodeController) GetRecache() {
	...
	sucNum, errNum := utils.RecacheCodes(id, c.CodeService)
